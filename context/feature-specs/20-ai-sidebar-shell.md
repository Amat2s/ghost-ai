Complete the existing AI sidebar placeholder and turn it into a proper floating chat sidebar component. The sidebar already exists. First, fix the floating placement and smooth slide-in behavior from the right side. Then, build out the sidebar UI inside it.

## Implementation

1. Fix the floating placement and smooth slide in behaviour
   - sidebar should float above the editor canvas
   - opening it should not push page content
   - slides in from the right
   - accepts `isOpen` prop

2. Separate the AI sidebar into its own component.
   - keep the open/close state controlled by the parent
   - preserve the slide animation, floating position, border, background, and shadow styling
...
4. Add a tabbed layout with two tabs.
...
7. Use the existing project color tokens.
...
- keep this focused on the sidebar UI structure and floating behaviour

## Check When Done

- AI sidebar is separated into its own component.
- Floating side-in behaviour is the same as the left side
